...
- 🔄 **自动日志解析**：支持直接解析原始日志文件（如 `.yaml` 或 `.log`），提取问答对
...
#### 🏥 医疗健康服务（乳腺癌康复支持）
- **用户分类**：患者、家属、医生、志愿者
- **话题分类**：症状管理、治疗咨询、手术康复、检查报告、情感支持
- **输出格式**：支持 YAML 格式以对接下游分析工具
...
# 📂 指定输出格式为 YAML
python run_analysis.py --full --output-format yaml
```
...
#### 原始日志文件支持
系统也支持直接解析特定格式的日志文件（如 `xyanb.yaml`），自动提取时间戳、用户提问（`[CHATGPT] query=`）和机器人回答（`[gewechat] Do send text to`）。只需将日志文件放入 `input/` 目录或通过 `--input-file` 指定。
